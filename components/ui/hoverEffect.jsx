import { cn } from "@/lib/utils";
import { EvervaultCard, Icon } from "./evervault-card";
import { useState } from "react";

export const HoverEffect = ({ items, className }) => {
    let [hoveredIndex, setHoveredIndex] = useState(null);

    return (
        <div className={cn("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-2", className)}>
            {items.map((item, idx) => (
                <div
                    key={item.id}
                    className="relative group block p-2 h-full w-full"
                    onMouseEnter={() => setHoveredIndex(idx)}
                    onMouseLeave={() => setHoveredIndex(null)}
                >
                    <div className="relative border border-white/[0.2] dark:border-white/[0.2] p-4">
                        {/* Corner Icons */}
                        <Icon className="absolute h-6 w-6 -top-3 -left-3 dark:text-white text-white" />
                        <Icon className="absolute h-6 w-6 -bottom-3 -left-3 dark:text-white text-white" />
                        <Icon className="absolute h-6 w-6 -top-3 -right-3 dark:text-white text-white" />
                        <Icon className="absolute h-6 w-6 -bottom-3 -right-3 dark:text-white text-white" />

                        {/* EvervaultCard with Image */}
                        <EvervaultCard src={item.src} alt={item.alt} />

                        {/* Name Display */}
                        <p className="text-center text-white mt-4 font-semibold text-2xl">
                            {item.alt}
                        </p>
                    </div>
                </div>
            ))}
        </div>
    );
};